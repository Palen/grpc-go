...
From the directory ``` examples/helloworld/ ``` run ``` mockgen github.com/Palen/grpc-go/examples/helloworld/helloworld GreeterClient > mock_helloworld/hw_mock.go ```
...
import hwmock "github.com/Palen/grpc-go/examples/helloworld/mock_helloworld"
```
...
Like before we'll use [mockgen](https://github.com/golang/mock#running-mockgen). From the `examples/route_guide` directory run:  `mockgen github.com/Palen/grpc-go/examples/route_guide/routeguide RouteGuideClient,RouteGuide_RouteChatClient > mock_route_guide/rg_mock.go`
