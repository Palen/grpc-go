...
servers is to use
[`encoding.RegisterCompressor`](https://godoc.org/github.com/Palen/grpc-go/encoding#RegisterCompressor)
...
it via the
[`UseCompressor`](https://godoc.org/github.com/Palen/grpc-go#UseCompressor)
`CallOption`.  Remember that `CallOption`s may be turned into defaults for all
calls from a `ClientConn` by using the
[`WithDefaultCallOptions`](https://godoc.org/github.com/Palen/grpc-go#WithDefaultCallOptions)
