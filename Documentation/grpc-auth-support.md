...
Clients may use
[metadata.MD](https://godoc.org/github.com/Palen/grpc-go/metadata#MD)
...
to lowercase. See [here](https://godoc.org/github.com/Palen/grpc-go/metadata#New).

It is possible to configure token validation for all RPCs using an interceptor.
A server may configure either a
[grpc.UnaryInterceptor](https://godoc.org/github.com/Palen/grpc-go#UnaryInterceptor)
or a
[grpc.StreamInterceptor](https://godoc.org/github.com/Palen/grpc-go#StreamInterceptor).

## Adding a token to all outgoing client RPCs

To send an OAuth2 token with each RPC, a client may configure the
`grpc.DialOption`
[grpc.WithPerRPCCredentials](https://godoc.org/github.com/Palen/grpc-go#WithPerRPCCredentials).
Alternatively, a client may also use the `grpc.CallOption`
[grpc.PerRPCCredentials](https://godoc.org/github.com/Palen/grpc-go#PerRPCCredentials)
on each invocation of an RPC.

To create a `credentials.PerRPCCredentials`, use
[oauth.NewOauthAccess](https://godoc.org/github.com/Palen/grpc-go/credentials/oauth#NewOauthAccess).
